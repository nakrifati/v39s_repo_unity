...
// ...дай мне сил переписать весь код этого проекта...
...
public class PlayerGUI : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (e.isMouse)
        {
            if (Physics.Raycast(ray, out hit, 100, 1 << 12 | 1 << 14))
            {
                ...
                if (clickGuiObj.GetComponent<StorageGUIStack>())
                    clickGuiObj = null;
            }
        ...
        if (one.recovery && two.GetComponent<VaultRoom>())
        {
            if (use)
                if (two.name == pi.GetMyRoom())
                    pi.Recovery(pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks));
            return true;
        }
        if (one.plant && two.GetComponent<Greenhouse>())
        {
            if (use)
                if (two.GetComponent<Greenhouse>().canGrow)
                    pc.Plant(currStack, pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks), two);
            return true;
        }
        if (one.eat && two.GetComponent<PlayerInfo>())
        {
            if (use)
                pc.Eat(currStack, pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks));
            return true;
        }
        if (one.suit && two.GetComponent<PlayerInfo>())
        {
            if (use)
                pi.DressSuit(pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks));
            return true;
        }
        if (one.weapon && two.GetComponent<PlayerInfo>())
        {
            if (use)
                pi.SetActiveWeapon(one);
            return true;
        }
    ...
    */
    ...
    {
        if (pi.Stacks[currStack].MyItems.Count == 0 || !pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).eat)
            return;
        pc.Plant(currStack, pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks), GameObject.FindGameObjectWithTag("Greenhouse"));
    ...
    public void GUIClickDrop()
    {
        ...
        {
            if (pi.Stacks[currStack].MyItems[0].weapon && pi.Stacks[currStack].MyItems[0].active)
                pi.DisableAllWeapons();

            pc.DropItem(currStack, pi.Stacks[currStack].GetName(), pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).id);
            inventoryMenu.gameObject.SetActive(false);
        }
        activeBlock = null;
        activeStack = null;
    ...
    {
        pi.DressSuit(pi.FindItem("Suit", pi.Stacks));
        activeBlock = null;
        clickGuiObj = null;
        inventoryMenu.gameObject.SetActive(false);
        pi.showSuit = true;
    ...
    public void GUIRClickStack() // Переписать! x_x
    {
        ...
        if (pi.Stacks[currStack].MyItems.Count > 0) // Если стек не пустой
        {
            ...
            {
                if (pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).plant) // Если это растение
                {
                    invtr.FindChild("Plant").gameObject.SetActive(true);
                    invtr.FindChild("Eat").gameObject.SetActive(true);
                }
                else if (pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).eat) //Если еда
                {
                    invtr.FindChild("Eat").gameObject.SetActive(true);
                }
                else if (pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).suit) // Если это костюм
                {
                    invtr.FindChild("DressSuit").gameObject.SetActive(true);
                    invtr.FindChild("RemoveSuit").gameObject.SetActive(true);
                }
                else if (pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).recovery) // Если это ремкомплект
                {
                    invtr.FindChild("Recovery").gameObject.SetActive(true);
                }
                else if (pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).weapon) // Если это оружие
                {
                    if (!pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).active)
                        invtr.FindChild("SetWeapon").gameObject.SetActive(true);
                    else
                        invtr.FindChild("RemoveWeapon").gameObject.SetActive(true);
                }
    ...
    {
        pi.DisableAllWeapons();
        pi.SetActiveWeapon(pi.Stacks[currStack].MyItems[0]);
    ...
    public void GUIClickRemoveWeapon()
    {
        pi.DisableAllWeapons();
    ...
    public void GUIDragStack()
    {
        ...
        {
            Debug.Log(pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).name + "     " + hit.transform.name);

            currObj = hit.collider;
            if (CheckUsable(pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks), hit.transform.gameObject, false) && currObj.transform.GetComponent<UseLight>())
    ...
    public void GUIDragUpCharacter()
    {
        activeBlock = null;
        activeStack = null;
        clickGuiObj = null;
    ...
    public void GUIDragUpStack()
    {
        ...
        if (Physics.Raycast(ray, out hit, 100))
        {
            ...
            if (!Use(pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks), hit.transform.gameObject))
            {
                if (hit.transform.name == "Stack")
                {
                    if (pi.Stacks[inventory.IndexOf(hit.transform.GetComponent<GUIBlock>())].MyItems.Count < 20)
                    {
                        if (pi.Stacks[inventory.IndexOf(hit.transform.GetComponent<GUIBlock>())].MyItems.Count > 0) // Если что-то есть
                        {
                            ...
                            if (pi.Stacks[inventory.IndexOf(hit.transform.GetComponent<GUIBlock>())].MyItems[0].name == pi.Stacks[currStack].MyItems[0].name && pi.Stacks[currStack].MyItems[0].pack)
                            {
                ...
                else if (hit.transform.name == "StorageGUIStack")
                {
                    if (pi.Stacks[currStack].MyItems[0].weapon && pi.Stacks[currStack].MyItems[0].active)
                        pi.DisableAllWeapons();
                    else if (pi.Stacks[currStack].MyItems[0].suit && pi.Stacks[currStack].MyItems[0].active)
                        pi.RemoveSuit();

                    GameObject clone = (GameObject)Instantiate(Resources.Load("Objects/" + pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks).name), hit.transform.position, Quaternion.identity);
                    clone.GetComponent<Item>().Ip = pi.FindLastItem(currStack, pi.Stacks[currStack].GetName(), pi.Stacks);
                    pi.Stacks[currStack].MyItems.RemoveAt(pi.Stacks[currStack].MyItems.Count - 1);
                    pc.Grab(clone, clone, hit.transform.GetComponent<StorageGUIStack>().st.Stacks);
                    hit.transform.GetComponent<StorageGUIStack>().st.UpdateStacksIcon();
                }
                else
                {
                    if (pi.Stacks[currStack].MyItems[0].weapon && pi.Stacks[currStack].MyItems[0].active)
