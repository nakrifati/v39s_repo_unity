        ...
        if (System.IO.File.Exists(fileName))
        {
            ...
            foreach (ObjectSaver obj in coll)
            {
                ...
                else if (pi) // Если игрок или бот
                {
                    ...
                    obj.GetGreenhouse(g);
                else if (g.GetComponent<Storage>())
                    obj.GetStorage(g.gameObject);
                else if (g.GetComponent<Computers>())
                    obj.GetComp(g.gameObject);
                else if (g.GetComponent<VaultEnvironment>())
                    obj.GetVaultEnv(g.gameObject);
...
public class ObjectSaver
{
    ...
    public class ComputersSL
    {
        ...
        public void GetCompData(GameObject prop)
        {
            prop.GetComponent<Computers>().cd = cd;
        }
    }

    [System.Serializable]
    public class StorageSL
    {
        List<PlayerInfo.MyStack> sda;
        public StorageSL(GameObject prop)
        {
            sda = prop.GetComponent<Storage>().Stacks;
        }

        public void GetStorage(GameObject prop)
        {
            prop.GetComponent<Storage>().Stacks = sda;
        }
    }
    ...
    [System.Serializable]
    public class VaultEnvironmentSL
    {
        public float EarthqTimer;

        public VaultEnvironmentSL(GameObject prop)
        {
            EarthqTimer = prop.GetComponent<VaultEnvironment>().EarthqTimer - Time.timeSinceLevelLoad;
        }

        public void GetVaultEnv(GameObject prop)
        {
            prop.GetComponent<VaultEnvironment>().EarthqTimer = EarthqTimer;
        }
    }
    ...
    public class PlayerInfoSL
    {
        ...
        public Item.ItemProp it, activeWeapon, activeSuit;
        ...
        public void GetSuit(GameObject receiver)
        {
            receiver.GetComponent<PlayerInfo>().activeSuit = activeSuit;
        }
    }
    ...
    public ObjectSaver(GameObject obj, string objectType)
    {
        ...
        else if (obj.GetComponent<Storage>())
            storage = new StorageSL(obj.gameObject);
        else if (obj.GetComponent<Computers>())
            comp = new ComputersSL(obj.gameObject);
        else if (obj.GetComponent<VaultEnvironment>())
            ve = new VaultEnvironmentSL(obj.gameObject);
    ...
    public void GetactiveWeapon(GameObject receiver)
    {
        receiver.GetComponent<PlayerInfo>().activeWeapon = player.activeWeapon;
    }
    public void GetActiveSuit(GameObject receiver)
    {
        receiver.GetComponent<PlayerInfo>().activeSuit = player.activeSuit;
    }
    public void GetItem(GameObject receiver)
    {
        item.GetItem(receiver);
    }
    public void GetVaultEnv(GameObject receiver)
    {
        receiver.GetComponent<VaultEnvironment>().EarthqTimer = ve.EarthqTimer;
    }
    ...
    public void GetStorage(GameObject receiver)
    {
        storage.GetStorage(receiver);
    }
    public void GetComp(GameObject receiver)
    {
        comp.GetCompData(receiver);
    }
}